...
# * CONNECT
engine = create_engine("sqlite:///bot/src/database/database files/sqlite/dev_db.db")
...
Base.metadata.create_all(engine)  # Создание всех таблиц если их нету

db = SessionLocal()

# * NEW VALUES
new_user = UserAllInfo(
    user_id=123456,
    is_bot=False,
    supports_online_queries=True,
    first_name="Shayden",
)
...
# * QUERY
user = db.query(UserAllInfo).filter(UserAllInfo.user_id == 123456).first()
print(user)
